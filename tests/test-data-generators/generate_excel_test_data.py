...
def generate_multi_sheet_excel():
    ...
    sheets = {
        'Employees': pd.DataFrame({
            ...
            'Department': ['Sales', 'IT', 'HR', 'Sales', 'IT', 'Finance', 'HR', 'Sales', 'IT', 'Finance'],
...
def generate_large_excel():
    ...
    data = {
        ...
        'Status': [random.choice(['Active', 'Inactive', 'Pending']) for _ in range(10000)]
    }
...
def generate_null_heavy_excel():
    ...
    df = pd.DataFrame({
        ...
        'RandomNulls': [i if random.random() > 0.7 else None for i in range(1, 101)]
    })
    return df
