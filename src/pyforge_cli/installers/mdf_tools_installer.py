...
class MdfToolsInstaller:
    ...
    def _setup_sql_server(self) -> bool:
        ...
        try:
            ...
            container = self._create_sql_container()
    ...
    def _create_sql_container(self):
        ...
        try:
            ...
            volume = self.docker_client.volumes.create("pyforge-sql-data")
        
        # Create MDF files volume
        try:
            mdf_volume = self.docker_client.volumes.get("pyforge-mdf-files")
        except docker.errors.NotFound:
            mdf_volume = self.docker_client.volumes.create("pyforge-mdf-files")
    ...
    def _wait_for_sql_server(self) -> bool:
        """Wait for SQL Server to be ready"""
        self.console.print("⏳ Waiting for SQL Server to start (this may take a minute)...")
    ...
    def start_container(self) -> bool:
        ...
        try:
            ...
            container = self._get_existing_container()
            if not container:
                self.console.print("❌ SQL Server container not found. Run installation first.")
                return False
    ...
    def show_logs(self, lines: int = 50) -> None:
        ...
        try:
            ...
            self.console.print(f"[bold]Last {lines} lines from SQL Server container:[/bold]")
