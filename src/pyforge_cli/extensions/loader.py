...
class ExtensionLoader:
    ...
    def load_all_extensions(self, timeout: float = DEFAULT_TIMEOUT) -> Dict[str, BaseExtension]:
    ...
    def _load_single_extension(
        ...
        try:
            ...
            if not extension_class.is_available():
                ...
                self._failed_extensions[name] = ExtensionLoadError("Dependencies not satisfied")
                return None
            ...
            logger.info(f"Successfully loaded extension: {name} ({self._loading_times[name]:.2f}s)")
            
            return extension
            
        except TimeoutError:
            error = ExtensionLoadError(f"Extension {name} initialization timed out after {timeout}s")
