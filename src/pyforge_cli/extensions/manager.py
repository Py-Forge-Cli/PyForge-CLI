...
from pyforge_cli.extensions.registry import ExtensionRegistry, ExtensionState
...
class ExtensionManager:
    ...
    def initialize(self, timeout: float = 30.0) -> None:
        ...
        try:
            ...
            logger.info(f"Extension manager initialized with {len(loaded_extensions)} extension(s)")
    ...
    def execute_hook(self, hook_name: str, *args, **kwargs) -> Dict[str, any]:
        ...
        for extension in self.get_all_extensions():
            if hasattr(extension, hook_name):
                try:
                    logger.debug(f"Executing hook {hook_name} on {extension.__class__.__name__}")
                    hook_method = getattr(extension, hook_name)
                    result = hook_method(*args, **kwargs)
                    results[extension.__class__.__name__] = result
                except Exception as e:
                    logger.error(f"Error executing hook {hook_name} on {extension.__class__.__name__}: {e}")
