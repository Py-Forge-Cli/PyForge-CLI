...
class ExtensionRegistry:
        ...
        with self._lock:
            if name not in self._extensions:
                self._extensions[name] = ExtensionMetadata(
                    name=name, entry_point=entry_point, state=ExtensionState.DISCOVERED
                )
                logger.debug(f"Registered discovered extension: {name}")

    def update_state(
        self, name: str, state: ExtensionState, error: Optional[Exception] = None
    ) -> None:
        ...
        with self._lock:
            if name in self._extensions:
                ...
                logger.warning(
                    f"Attempted to update state for unknown extension: {name}"
                )
        ...
        with self._lock:
            if name in self._extensions:
                ...
                metadata.description = (
                    description or getattr(instance, "__doc__", "").strip()
                )

                # Automatically enable loaded extensions
                self._enabled_extensions.add(name)
                logger.info(
                    f"Registered loaded extension: {name} (v{metadata.version})"
                )
            else:
    ...
    def get_extensions_by_state(self, state: ExtensionState) -> List[str]:
        ...
        with self._lock:
            return [
                name
                for name, metadata in self._extensions.items()
