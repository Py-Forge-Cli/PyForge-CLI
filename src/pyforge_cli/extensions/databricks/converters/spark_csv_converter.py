...
from pyspark.sql.functions import col, regexp_replace, to_date, to_timestamp, trim
...
class SparkCSVConverter:
    ...
    def __init__(self, spark_session: Optional[SparkSession] = None):
        ...
        self.logger = logging.getLogger(
            "pyforge.extensions.databricks.converters.spark_csv"
        )
    ...
    def convert(
        ...
        try:
            ...
            self.logger.info(
                f"Starting Spark CSV conversion: {input_path} -> {output_path}"
            )
            ...
            use_streaming = options.get(
                "streaming", False
            ) or self._should_use_streaming(input_path, options)
            ...
            if success:
                self.logger.info(
                    f"Successfully converted {input_path} to {output_format}"
                )
            else:
    ...
    def _apply_custom_transformations(
        self, df: DataFrame, transformations: List[Dict[str, Any]]
    ) -> DataFrame:
        """Apply custom transformations."""
    ...
    def _optimize_dataframe(
        self, df: DataFrame, output_format: str, options: Dict[str, Any]
    ) -> DataFrame:
