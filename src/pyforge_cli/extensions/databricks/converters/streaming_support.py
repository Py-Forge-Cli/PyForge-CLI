...
class StreamingSupport:
    ...
    def process_file_streaming(
        ...
        try:
            ...
            stream_df = self._create_stream_reader(input_path, input_format, options)

            if stream_df is None:
                return False

            # Apply transformations
            stream_df = self._apply_streaming_transformations(stream_df, options)

            # Start streaming query
            query = self._create_stream_writer(
                stream_df, output_path, output_format, query_name, options
            )
    ...
    def _create_stream_reader(
        self, path: str, format: str, options: Dict[str, Any]
    ) -> Optional[DataFrame]:
        ...
        try:
            ...
            if format == "csv":
                ...
                reader = reader.option(
                    "inferSchema", options.get("infer_schema", "true")
                )
                reader = reader.option(
                    "maxFilesPerTrigger", options.get("files_per_batch", 1)
                )
    ...
    def _create_stream_writer(
        ...
        try:
            ...
            checkpoint_path = options.get(
                "checkpoint_location", f"{path}_checkpoint_{query_name}"
            )

            writer = (
                df.writeStream.outputMode(options.get("output_mode", "append"))
                .format(format)
                .option("path", path)
    ...
    def _wait_for_completion(
        self, query: StreamingQuery, options: Dict[str, Any]
    ) -> bool:
    ...
    def _start_progress_tracking(self, query: StreamingQuery, callback: Callable):
    ...
    def estimate_file_size_for_streaming(
        self, file_path: Union[str, Path], format: str
    ) -> Dict[str, Any]:
        """
    ...
    def get_stream_statistics(self, query_name: str) -> Optional[Dict[str, Any]]:
        """
