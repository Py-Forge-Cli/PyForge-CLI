...
from pyspark.sql.functions import (
    col, explode, explode_outer, posexplode, flatten, 
    when, concat_ws, struct, array, lit, regexp_replace,
...
class SparkXMLConverter:
    ...
    def __init__(self, spark_session: Optional[SparkSession] = None):
        ...
        """
        self.logger = logging.getLogger("pyforge.extensions.databricks.converters.spark_xml")
    ...
    def convert(
        ...
        try:
            ...
            self.logger.info(f"Starting Spark XML conversion: {input_path} -> {output_path}")
        ...
        for col_name, col_type in nested_cols.items():
            if isinstance(col_type, StructType):
                ...
                if array_handling == "explode":
                    flattened_df = flattened_df.withColumn(col_name, explode_outer(col(col_name)))
                elif array_handling == "first":
                    flattened_df = flattened_df.withColumn(
                        col_name,
                        when(col(col_name).isNotNull(), col(col_name)[0])
                    )
                elif array_handling == "json":
                    flattened_df = flattened_df.withColumn(col_name, to_json(col(col_name)))
    ...
    def _expand_struct(self, df: DataFrame, col_name: str, options: Dict[str, Any]) -> DataFrame:
        ...
        for field in struct_schema.fields:
            ...
            expanded_cols.append(
                col(f"{col_name}.{field.name}").alias(new_col_name)
            )
    ...
    def _expand_path(self, df: DataFrame, path: str, options: Dict[str, Any]) -> DataFrame:
