...
from pyspark.sql.functions import col, lit, concat_ws, when, trim, to_timestamp
...
class SparkExcelConverter:
    ...
    def __init__(self, spark_session: Optional[SparkSession] = None):
        ...
        """
        self.logger = logging.getLogger("pyforge.extensions.databricks.converters.spark_excel")
    ...
    def convert(
        ...
        try:
            ...
            self.logger.info(f"Starting Spark Excel conversion: {input_path} -> {output_path}")
            
            # Detect sheets
            sheet_info = self._detect_sheets(input_path)
            self.logger.info(f"Detected {len(sheet_info)} sheets: {list(sheet_info.keys())}")
    ...
    def _detect_header(self, df: Any) -> bool:
        ...
        first_row_types = set(type(val).__name__ for val in df.iloc[0])
        data_types = set()
        ...
        if first_row_types == {'str'} and ('int' in data_types or 'float' in data_types):
            return True
    ...
    def _process_sheets(
        ...
        if not options.get("include_empty_sheets", False):
            sheet_info = {
                name: info for name, info in sheet_info.items()
        ...
        for sheet_name in sheet_info:
            df = self._read_sheet(excel_path, sheet_name, sheet_info[sheet_name], options)
            if df is not None:
    ...
    def _clean_column_names(self, df: DataFrame) -> DataFrame:
        ...
        for old_name in df.columns:
            ...
            if old_name.startswith("_") or re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', old_name):
                continue
    ...
    def _convert_excel_dates(self, df: DataFrame) -> DataFrame:
        ...
        for field in df.schema.fields:
            if field.dataType == DoubleType() and 'date' in field.name.lower():
                ...
                df = df.withColumn(
                    field.name,
                    when(col(field.name) > 25569,  # Unix epoch in Excel serial
    ...
    def _schemas_compatible(self, schemas: List[StructType]) -> bool:
        ...
        for schema in schemas:
            cols = set(f.name for f in schema.fields if f.name != "_sheet_name")
