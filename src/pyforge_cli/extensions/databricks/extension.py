...
class DatabricksExtension(BaseExtension):
        ...
        try:
            ...
            if self.environment.is_databricks_environment():
                ...
                if not env_info.get("has_pyspark"):
                    self.logger.warning("PySpark not available - some features disabled")
            else:
                self.logger.info("Non-Databricks environment - running in compatibility mode")
    ...
    def enhance_convert_command(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if env_info["is_databricks"]:
            ...
            if env_info["compute_type"] == "serverless":
                ...
                options.setdefault("photon_enabled", env_info.get("serverless_features", {}).get("photon_acceleration", False))
            
            # Unity Catalog Volume detection
            input_path = options.get("input_file", "")
            if isinstance(input_path, (str, Path)) and str(input_path).startswith("/Volumes"):
                options["is_volume_path"] = True
                options["volume_info"] = self.volume_ops.parse_volume_path(str(input_path))
    ...
    def hook_error_handling(
        ...
        try:
            ...
            if isinstance(error, MemoryError) and self.environment.is_databricks_environment():
                self.logger.info("Memory error in Databricks - suggesting streaming mode")
    ...
    def hook_performance_optimization(
        ...
        """
        try:
            if operation != "convert" or not self.environment.is_databricks_environment():
                return options
    ...
    def _env_info_command(self, **kwargs) -> bool:
        ...
        try:
            ...
            if env_info["is_databricks"]:
                ...
                print(f"   Unity Catalog: {'✅' if env_info['has_unity_catalog'] else '❌'}")
                print(f"   PySpark Available: {'✅' if env_info['has_pyspark'] else '❌'}")
            ...
            if valid:
                ...
                if volume_info:
                    ...
                    if volume_info.is_managed is not None:
                        print(f"   Type: {'Managed' if volume_info.is_managed else 'External'}")
            else:
