...
class VolumeOperations:
    ...
    def parse_volume_path(self, path: str) -> Optional[VolumeInfo]:
        ...
        return VolumeInfo(catalog=catalog, schema=schema, volume=volume, path=file_path)
        ...
        try:
            ...
            try:
                ...
                if volume_info.schema not in schemas:
                    return (
                        False,
                        f"Schema '{volume_info.schema}' not found in catalog '{volume_info.catalog}'",
                    )
            except Exception as e:
    ...
    def list_volume_contents(
        self, volume_path: str, recursive: bool = False, pattern: Optional[str] = None
    ) -> List[VolumeFile]:
        """
    ...
    def copy_to_volume(
        self, source_path: str, volume_path: str, overwrite: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if dbutils:
                ...
                return VolumeFile(
                    ...
                    extension=(
                        Path(file_info.name).suffix if not file_info.isDir() else None
                    ),
                )
            else:
                # Fallback to OS
    ...
    def _list_with_dbutils(
        self, path: str, recursive: bool, pattern: Optional[str]
    ) -> List[VolumeFile]:
                ...
                for item in items:
                    files.append(
                        VolumeFile(
                            ...
                            extension=(
                                Path(item.name).suffix if not item.isDir() else None
                            ),
                        )
                    )
                ...
                for item in os.listdir(path):
                    ...
                    files.append(
                        VolumeFile(
                            ...
                            extension=(
                                Path(item).suffix if os.path.isfile(full_path) else None
                            ),
                        )
                    )
