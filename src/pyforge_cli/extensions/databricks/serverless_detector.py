...
class ServerlessDetector:
    ...
    def is_serverless(self) -> bool:
        ...
        if is_serverless:
            self.logger.info(f"Serverless compute confirmed with signals: {detection_signals}")
        else:
            self.logger.info(f"Not serverless compute. Signals found: {detection_signals}")
            ...
            for config_key, expected_value in self.SERVERLESS_SPARK_CONFIGS.items():
                try:
                    ...
                    if actual_value == expected_value.lower():
                        matches += 1
                        self.logger.debug(f"Spark config match: {config_key}={actual_value}")
                except Exception:
                    continue
    ...
    def _check_process_characteristics(self) -> bool:
        ...
        try:
            ...
            if any(marker in cmdline.lower() for marker in ["serverless", "spark-connect"]):
                return True
    ...
    def get_detection_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "environment_details": {
                "runtime_version": os.environ.get("DATABRICKS_RUNTIME_VERSION", "Not found"),
