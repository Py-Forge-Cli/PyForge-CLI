...
class ConverterSelector:
    ...
    FORMAT_SUPPORT = {
        ConverterType.SPARK: {
            "csv", "json", "parquet", "orc", "avro", "xml", "delta", "text"
        },
        ConverterType.PANDAS: {
            "csv", "json", "parquet", "excel", "xlsx", "xls", "feather", "hdf", "stata"
        },
        ConverterType.PYARROW: {
            "parquet", "feather", "csv", "json", "orc"
        },
        ConverterType.NATIVE: {
            "csv", "json", "parquet", "excel", "xml", "yaml", "toml", "avro"
        }
    }
    ...
    def _make_selection(
        ...
        for converter_type, supported_formats in self.FORMAT_SUPPORT.items():
            if file_chars.format in supported_formats and output_format in supported_formats:
                scores[converter_type] += 0.2
            elif file_chars.format in supported_formats or output_format in supported_formats:
        ...
        confidence = min(scores[best_converter] / sum(scores.values()) if sum(scores.values()) > 0 else 0.0, 1.0)
        
        # Determine performance characteristics
        estimated_performance = self._estimate_performance(best_converter, file_chars, env_context)
    ...
    def _is_nested_structure(self, obj: Any, depth: int = 0, max_depth: int = 3) -> bool:
