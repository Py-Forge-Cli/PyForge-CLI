...
class DualBackendMDBReader:
        ...
        if ucanaccess.is_available():
            ...
            try:
                if ucanaccess.connect(db_path, password):
                    ...
                    self.logger.warning("UCanAccess connection failed, trying fallback...")
    ...
    def _raise_connection_error(self, attempts: List[tuple], db_path: str):
        ...
        if not available_backends:
            ...
            error_msg += "  • Ensure the database is not currently open in Microsoft Access\n"
    ...
    def list_tables(self) -> List[str]:
        ...
        try:
            ...
            if tables:
                ...
                if space_tables:
                    self.logger.info(f"Found {len(space_tables)} tables with spaces in names: {space_tables}")
    ...
    def read_table(self, table_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(f"Error reading table '{table_name}' with {self.backend_name}: {e}")
            raise
    ...
    def close(self):
        ...
        if self.backend:
            try:
                ...
                self.logger.warning(f"Error closing {self.backend_name} connection: {e}")
            finally:
                self.backend = None
                self.backend_name = None
                self._connection_info = {}
